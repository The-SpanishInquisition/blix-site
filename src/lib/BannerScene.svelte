...
{#each boxes as box, i}
  <T.Mesh
    position={[box.pos[0], 0.5 * box.dim - 1, box.pos[1]]}
    rotation={box.rot}
    castShadow
    receiveShadow
  >
...
<SheetObject
  ...
  <T.Mesh
    position={[UserBox.pos[0], 0.5 * UserBox.dim - 1, UserBox.pos[1]]}
    rotation={UserBox.rot}
    castShadow
    receiveShadow
  >
    {#if UserBox?.blixBox}
      <BlixBox
        dim={UserBox.dim}
        position={UserBox.pos}
        rotation={UserBox.rot}
      />
    {/if}
    <RoundedBoxGeometry
      args={[UserBox.dim, UserBox.dim, UserBox.dim]}
      radius={UserBox.dim * 0.1}
    />
